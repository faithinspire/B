...
import { Search, MapPin, Star, Loader, Filter, X, ChevronDown } from 'lucide-react';
...
export default function SearchPage() {
  ...
  const initialCountry = (searchParams.get('country') || 'NG').toUpperCase();
  ...
  const countryName = country === 'US' ? '🇺🇸 USA' : '🇳🇬 Nigeria';
  const paymentProvider = country === 'US' ? 'Stripe' : 'Paystack';
        ...
        <div className="relative h-44 bg-gradient-to-br from-purple-100 to-pink-100 overflow-hidden">
          ...
          <div
            className={`absolute top-2 left-2 px-2 py-0.5 rounded-full text-xs font-bold text-white ${
              isBarber ? 'bg-blue-600' : 'bg-purple-600'
            }`}
          >
          ...
          <div
            className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-xs font-bold text-white ${
              pro.payment_provider === 'stripe' ? 'bg-blue-500' : 'bg-green-600'
            }`}
          >
            {pro.payment_provider === 'stripe' ? '💳 Stripe' : '💰 Paystack'}
          </div>
          {/* Verification badge */}
          {(pro.verification_status === 'verified' || pro.verification_status === 'approved') && (
            <div className="absolute bottom-2 left-2 bg-green-500 text-white text-xs px-1.5 py-0.5 rounded-full font-semibold">
              ✓ Verified
            </div>
          )}
        </div>
        ...
        <div className="p-3 flex-1 flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-1 mt-auto pt-2 border-t border-gray-100">
            <button
              onClick={(e) => {
                e.stopPropagation();
                router.push(`/braider/${pro.user_id || pro.id}`);
              }}
              className={`py-2 rounded-lg text-xs font-semibold transition-colors ${
                isBarber ? 'bg-blue-50 text-blue-600 hover:bg-blue-100' : 'bg-purple-50 text-purple-600 hover:bg-purple-100'
              }`}
            >
              👤 Profile
            </button>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-pink-50 pb-24">
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-700 to-pink-600 text-white py-8 px-4 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-2xl sm:text-3xl font-serif font-bold mb-4">
            {countryName} - {profession ? (profession === 'braider' ? 'Braiders' : 'Barbers') : 'Professionals'}
          </h1>
          <p className="text-purple-100 text-sm mb-4">
            Payments via {paymentProvider} • {professionals.length} professional{professionals.length !== 1 ? 's' : ''} found
          </p>
          ...
          <>
            ...
            {braiders.length > 0 && (
              <div className="mb-10">
                <h2 className="text-xl font-serif font-bold text-gray-900 mb-4">
                  ✂️ Braiders ({braiders.length})
                </h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {braiders.map((pro, idx) => (
                    <ProfCard key={pro.id} pro={pro} idx={idx} />
                  ))}
                </div>
              </div>
            )}
            ...
            {barbers.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {barbers.map((pro, idx) => (
                    <ProfCard key={pro.id} pro={pro} idx={idx + braiders.length} />
                  ))}
                </div>
              </div>
            )}
