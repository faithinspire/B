...
import { NIGERIAN_STATES, IDENTIFICATION_TYPES, NigerianState } from '@/lib/nigerian-locations';
...
interface BarberSignupFormProps {
  onSuccess?: () => void;
}
...
export function BarberSignupForm({ onSuccess }: BarberSignupFormProps) {
  ...
  const validateBasicInfo = () => {
    ...
    if (!validatePhoneNumber(phone, country)) { setError('Invalid phone number for selected country'); return false; }
  ...
  const validateLocation = () => {
    ...
    if (!city.trim()) { setError('City/Town is required'); return false; }
    if (!address.trim()) { setError('Address is required'); return false; }
    if (!nextOfKinName.trim()) { setError('Next of kin name is required'); return false; }
    if (!nextOfKinPhone.trim()) { setError('Next of kin phone is required'); return false; }
    if (!nextOfKinRelationship.trim()) { setError('Next of kin relationship is required'); return false; }
    return true;
  };

  const validateProfessional = () => {
    if (!specialization) { setError('Specialization is required'); return false; }
    if (!yearsExperience) { setError('Years of experience is required'); return false; }
    if (selectedServices.length === 0) { setError('Select at least one service'); return false; }
    if (!bio.trim()) { setError('Professional bio is required'); return false; }
    return true;
  };

  const validateIdentification = () => {
    if (!idType) { setError('ID type is required'); return false; }
    if (!idNumber.trim()) { setError('ID number is required'); return false; }
    if (!idDocument) { setError('ID document is required'); return false; }
    return true;
  };
    ...
    try {
      ...
      if (idDocument) {
        try {
          ...
          const uploadRes = await fetch('/api/upload/braider-id', { method: 'POST', body: formData });
          if (uploadRes.ok) {
      ...
      // Create account — role is 'braider' (shared dashboard) but profession_type is 'barber'
      ...
      try {
        await fetch('/api/auth/ensure-profile', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: data.user.id, email, fullName, role: 'braider' }),
        });
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between mb-8">
        {steps.map((step, idx) => (
          <div key={step.id} className="flex items-center flex-1">
            ...
            >
              {completedSteps.includes(step.id) ? <CheckCircle className="w-5 h-5" /> : idx + 1}
            </button>
            {idx < steps.length - 1 && (
              <div className={`flex-1 h-1 mx-2 ${completedSteps.includes(step.id) ? 'bg-green-500' : 'bg-gray-200'}`} />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-lg p-6 space-y-4">
        ...
        {currentStep === 'basic' && (
          <>
            ...
            <PhoneInput value={phone} onChange={setPhone} country={country} label="Phone Number" required />
            <div>
        ...
        {currentStep === 'location' && (
          <>
            ...
            <div>
              ...
              <select value={state} onChange={e => { setState(e.target.value as NigerianState); setCity(''); }}
                className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-blue-600">
            ...
            <div>
              ...
              <textarea value={address} onChange={e => setAddress(e.target.value)} placeholder="Your barbershop or home address" rows={3}
                className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-blue-600" />
            </div>
            <div className="border-t-2 border-gray-200 pt-4 mt-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Full Name *</label>
                <input type="text" value={nextOfKinName} onChange={e => setNextOfKinName(e.target.value)} placeholder="Emergency contact name"
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-blue-600" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Phone Number *</label>
                <input type="tel" value={nextOfKinPhone} onChange={e => setNextOfKinPhone(e.target.value)} placeholder="Emergency contact phone"
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-blue-600" />
              </div>
              <div>
                ...
                <select value={nextOfKinRelationship} onChange={e => setNextOfKinRelationship(e.target.value)}
                  ...
                  {['Parent','Sibling','Spouse','Child','Friend','Other'].map(r => <option key={r} value={r.toLowerCase()}>{r}</option>)}
                </select>
              </div>
            </div>
          </>
          ...
          <>
            ...
            <div>
              ...
              <select value={specialization} onChange={e => setSpecialization(e.target.value)}
                ...
                <option value="afro-taper">Afro Taper</option>
                <option value="waves">Waves</option>
                <option value="dreadlocks">Dreadlocks</option>
                <option value="lineup">Line Up / Edge Up</option>
                <option value="beard">Beard Grooming</option>
                <option value="general">General Barbering</option>
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <textarea value={bio} onChange={e => setBio(e.target.value)}
                placeholder="Tell customers about your experience, style, and what makes you unique..." rows={4}
                className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-blue-600" />
            </div>
          </>
        )}
          ...
          <>
            ...
            <div>
              ...
              <select value={idType} onChange={e => setIdType(e.target.value)}
                ...
                {IDENTIFICATION_TYPES.map(type => <option key={type} value={type}>{type}</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-600 transition-colors">
                <input type="file" accept="image/*,.pdf" onChange={e => e.target.files?.[0] && setIdDocument(e.target.files[0])}
                  className="hidden" id="id-upload-barber" />
                <label htmlFor="id-upload-barber" className="cursor-pointer">
                  <p className="text-sm text-gray-600">{idDocument ? idDocument.name : 'Click to upload or drag and drop'}</p>
          ...
          <>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-gray-700"><strong>Profession:</strong> Barber 💈</p>
      ...
      <div className="flex gap-3">
        <button onClick={handlePrevious} disabled={currentStep === 'basic' || loading}
